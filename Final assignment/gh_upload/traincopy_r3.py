...
#from modelcopy9 import Model
from modelcopy15 import Model
...
class FocalDiceLoss(nn.Module):
    def __init__(self, num_classes=19, gamma=2.0, alpha=0.25, ignore_index=255):
    ...
    def forward(self, logits, targets):
        ...
        # Create one-hot targets, excluding the ignore_index
        target_one_hot = F.one_hot(torch.clamp(targets, 0, self.num_classes - 1), 
                                   num_classes=self.num_classes).permute(0, 3, 1, 2).float()
        
        # Apply mask to both probs and one-hot
        ...
        dice_score = (2. * intersection + 1) / (cardinality + 1)
        dice_loss = 1 - dice_score.mean()

        # Weighted Sum (adjust lambdas based on performance)
...
def get_dice_score(logits, targets, num_classes=19, ignore_index=255):
    probs = F.softmax(logits, dim=1)
    predictions = probs.argmax(dim=1)
    
    # Create mask to ignore void pixels
    mask = (targets != ignore_index)
    
    dice_per_class = []
    for c in range(num_classes):
        pred_c = (predictions == c) & mask
        true_c = (targets == c) & mask
        
        intersection = (pred_c & true_c).sum().item()
        cardinality = pred_c.sum().item() + true_c.sum().item()
        
        if cardinality == 0:
            # Class not present in this batch; ignore it
            dice_per_class.append(float('nan'))
        else:
            dice_per_class.append((2. * intersection) / cardinality)
...
def main(args):
    ...
    train_transforms = A.Compose([
        ...
        ], p=0.4), # Only 40% of images get weather to keep training stable
        A.OneOf([
            A.GaussNoise(var_limit=(10.0, 50.0), p=1.0),
            A.ImageCompression(quality_lower=60, quality_upper=100, p=1.0),
            A.MotionBlur(blur_limit=7, p=1.0),
        ], p=0.3),
        A.ShiftScaleRotate(shift_limit=0.05, scale_limit=0.2, rotate_limit=5, p=0.1),
    ...
    criterion = FocalDiceLoss(num_classes=19, ignore_index=255).to(device)
    ...
    optimizer = torch.optim.AdamW([
        {'params': encoder_params, 'lr': 5e-5}, # Group 0: ResNet Encoder
        {'params': decoder_params, 'lr': 5e-4}  # Group 1: Decoder, ASPP, Attention
    ], weight_decay=0.01)

    # 3. Setup OneCycleLR (Ensure max_lr order matches the groups above)
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        optimizer,
        max_lr=[1e-4, 1e-3], # [Group 0 Max, Group 1 Max]
        epochs=args.epochs,
    ...
    # 1. Map IDs (Do this on CPU or GPU, but Tensor mapping is best)
        ...
        with torch.no_grad():
            ...
            for i, (images, labels) in enumerate(valid_dataloader):
                ...
                batch_dice = get_dice_score(outputs, labels)
            ...
            epoch_dice = torch.stack(epoch_dice)
            mean_dice_per_class = torch.nanmean(epoch_dice, dim=0)
            overall_mean_dice = torch.nanmean(mean_dice_per_class).item()            
  ...
  # Zero out gradients for the next iteration
            ...
            wandb.log({
                "val_loss": valid_loss,
                "val_mean_dice": overall_mean_dice,
                "dice_sky": mean_dice_per_class[10], # Assuming Sky is index 10
                "dice_nature": mean_dice_per_class[8], # Assuming Vegetation is index 8
                "epoch": epoch + 1
            }, step=(epoch + 1) * len(train_dataloader))
            
        # SAVE BEST MODEL BASED ON DICE
            if overall_mean_dice > best_mean_dice: # Track best_mean_dice instead of loss
                best_mean_dice = overall_mean_dice
                if current_best_dice_path:
                    os.remove(current_best_dice_path)
                current_best_dice_path = os.path.join(
                    output_dir, 
                    f"best_dice-epoch={epoch:04}-val_loss={valid_loss:04}.pt"
                )
                torch.save(model.state_dict(), current_best_dice_path)
                print(f"New Best Model Saved with Mean Dice: {overall_mean_dice:.4f}")
