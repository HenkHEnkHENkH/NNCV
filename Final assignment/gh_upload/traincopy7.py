...
from modelcopy10 import Model

def compute_class_weights(dataset, num_classes=19, ignore_index=255):
    ...
    class_weights = 1.0 / torch.log(class_counts + 1.05)
...
class DiceCELoss(nn.Module):
    ...
    """
    def __init__(self, num_classes=19, ignore_index=255, class_weights=None):
    ...
    def forward(self, logits, targets):
        ...
        ce_loss = F.cross_entropy(logits, targets, weight=self.class_weights, ignore_index=self.ignore_index)
...
def main(args):
    ...
    train_transforms = A.Compose([
        A.RandomResizedCrop(size=(256, 512), scale=(0.8, 1.0), ratio=(0.75, 1.33), p=1.0),
        A.HorizontalFlip(p=0.5),
        A.ShiftScaleRotate(
            shift_limit=0.05, scale_limit=0.2, rotate_limit=5, p=0.5
        ),
    ...
    criterion = DiceCELoss(num_classes=19, ignore_index=255, class_weights=class_weights)
